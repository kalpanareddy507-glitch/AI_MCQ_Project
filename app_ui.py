...
# =========================================================
# DATABASE INTEGRATION & SCHEMA AUTO-REPAIR
# =========================================================
...
# NEW helper function to detect host name automatically
def get_base_url():
    """Generates base context URL to work seamlessly across localhost or cloud servers."""
    try:
        # Tries to fall back safely to Streamlit standard patterns if metadata headers fail
...
if review_mode and view_type == "host":
    ...
    cursor.execute("SELECT questions, points_per_question FROM exams WHERE exam_id=?", (review_mode,))
    ...
    for name, score in submissions_list:
        st.markdown(f"""
        <div class="sheet-row">
            <div>Candidate Username: <strong>{name}</strong></div>
...
# =========================================================
    ...
    if is_admin:
        ...
        # Dynamically map links to network base path
...
# =========================================================
    ...
    if st.button("Generate Secure Exam Suite", type="primary"):
        ...
        cursor.execute("""
        INSERT INTO exams (exam_id, username, password, questions, created_at, expires_at, consumed, exam_duration, student_answers, target_students, points_per_question) 
        VALUES (?,?,?,?,?,?,?,?, ?, ?, ?)
        """, (exam, "MULTI_STUDENT", json.dumps(passwords_matrix), json.dumps(qs), now, now + 3600*2, 0, int(num_q)*45, json.dumps({}), int(student_headcount), float(fixed_score_weight)))
        conn.commit()

        # Dynamic mapping for sharing setup urls on the fly
        ...
        st.markdown(f"""
        <div class="token-box">
        <b>Shared Testing URL for Students:</b> <code>{student_link}</code>
        </div>
        """, unsafe_allow_html=True)
...
# =========================================================
    ...
    cursor.execute("SELECT exam_id, username, password, questions, created_at, expires_at, consumed, exam_duration, student_answers, target_students, points_per_question FROM exams WHERE exam_id=?", (exam_id,))
    ...
    (eid, group_name, password_matrix_json, qs_json, created, expires, consumed, duration, raw_answers, target_students, points_per_question) = data
    ...
    if not st.session_state.auth:
        ...
        for i, q in enumerate(qs):
            ...
            current_selected = st.session_state.answers.get(i, None)
            for idx, option in enumerate(q["options"]):
                box_key = f"q_{i}_idx_{idx}_opt_{option}"
                should_check = (current_selected == option)
                is_checked = st.checkbox(option, value=should_check, key=box_key)
                
                if is_checked and current_selected != option:
                    st.session_state.answers[i] = option
                    st.rerun()
                elif not is_checked and current_selected == option:
